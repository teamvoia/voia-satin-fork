    ...
    for (int j = 0; j <= edgeZ; j++) {
        ...
        for (int i = 0; i < perLoop; i++) {
            ...
            if (j != edgeZ) {
                ...
                ind[triIndex] = (TriangleIndices) { (uint32_t)i0, (uint32_t)i2, (uint32_t)i3 };
                triIndex++;
                ind[triIndex] = (TriangleIndices) { (uint32_t)i0, (uint32_t)i3, (uint32_t)i1 };
                triIndex++;
            }
        }
...
void connectPatchEdges(GeometryData *dst, int n, int firstPatchIndex, int secondPatchIndex,
    ...
    if (firstPatchEdge == PatchEdgeLeft && secondPatchEdge == PatchEdgeRight) {
        uint32_t i0 = (uint32_t)firstOffset;
        uint32_t i3 = (uint32_t)secondOffset + nMinusOne;
        for (uint32_t row = 0; row < nMinusOne; row++) {
        ...
        uint32_t i0 = (uint32_t)firstOffset + nMinusOne;
...
void connectPatchCorners(GeometryData *dst, int n, int firstPatchIndex, int secondPatchIndex,
    ...
    const uint32_t verticesPerPatch = n * (n + 1) / 2;
    const uint32_t triangles = 2;
